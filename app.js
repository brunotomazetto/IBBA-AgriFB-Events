...
async function saveUserToDb(user) {
  ...
  if (isOnline()) {
    const { error } = await _supabase.from('users').upsert({ username: user.username, display_name: user.displayName, initials: user.initials, role: user.role, password_hash: user.passwordHash }, { onConflict: 'username' });
...
function subscribeRealtime() {
  ...
  realtimeSub = _supabase.channel('crm')
    .on('postgres_changes', { event:'*', schema:'public', table:'events' }, p => {
      if (p.eventType==='DELETE') { events = events.filter(e => e.id !== p.old.id); }
      else { const ev=dbToEv(p.new); const i=events.findIndex(e=>e.id===ev.id); if(i>=0)events[i]=ev;else events.unshift(ev); }
      saveLocal(); renderDashboard();
      if (document.getElementById('page-events').classList.contains('active')) renderEvents();
      showSyncPulse();
    }).subscribe();
}
function showSyncPulse() { const el=document.getElementById('syncDot'); if(el){el.classList.add('pulse');setTimeout(()=>el.classList.remove('pulse'),1200);} }
...
function starHTML(value, field, id) {
  ...
  for (let i=1;i<=5;i++) h+=`<span class="star${i<=(value||0)?' on':''}" onclick="setRating('${id}','${field}',${i})" onmouseover="hoverStars(this,${i})" onmouseout="resetStars(this)">★</span>`;
  return h+'</div>';
}
...
function updateModalStars(fid) {
  ...
  document.getElementById(fid+'-stars')?.querySelectorAll('.star-btn').forEach((s,i)=>{s.classList.toggle('on',i<val);s.textContent=i<val?'★':'☆';});
}
...
function setPage(p) {
  ...
  // highlight top nav for admin pages
  const topPages=['connect','users','settings'];
  if(topPages.includes(p)){
...
function getDashFiltered() {
  ...
  return events.filter(e => {
    ...
    if (month) {
      // filter by month of registeredDate
...
function renderDashboard() {
  ...
  document.getElementById('quickGrid').innerHTML=[['All',total],...STATUSES.map(s=>[s,counts[s]])].map(([s,n])=>`
    <div class="qcard${activeFilter===s?' active-card':''}" onclick="goFilter('${s}')">
      <div class="qicon" style="background:${s==='All'?'#f0f0f0':STATUS_BG[s]};color:${s==='All'?'#333':STATUS_COLORS[s]}">${icons[s]||''}</div>
      <div class="qcount">${n}</div>
      <div class="qlabel">${s==='All'?'Total':s}</div>
    </div>`).join('');
  ...
  document.getElementById('pipelineChart').innerHTML=STATUSES.map(s=>`
    ...
    <div class="bar-num">${counts[s]}</div></div>`).join('');
  ...
  document.getElementById('recentBody').innerHTML=recent.map(e=>{
    ...
    return `<tr onclick="editEvent('${e.id}')">
      <td><span class="evt-id">${e.id}</span></td><td style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${e.name}</td>
      ...
      <td>${ratingBadge(e.ratingPreCall)}</td><td>${ratingBadge(e.ratingCall)}</td>
    </tr>`;
  ...
  document.getElementById('fupList').innerHTML=fups.length
    ?fups.map(e=>`<div class="att-item" onclick="editEvent('${e.id}')"><div class="att-dot" style="background:#e65100"></div><div><div class="att-name">${e.name}</div><div class="att-meta">${e.id} · ${computeSLA(e)} days</div></div></div>`).join('')
...
function sortArrow(field){
  ...
  return sortDir==='asc'?'<span style="color:var(--orange);margin-left:2px">↑</span>':'<span style="color:var(--orange);margin-left:2px">↓</span>';
}
...
function renderEvents(){
  ...
  document.querySelectorAll('.dtbl th.sortable').forEach(th=>{
    const f=th.onclick?.toString().match(/setSort\('(\w+)'\)/)?.[1];
    const sa=th.querySelector('.sa');
    if(sa&&f)sa.innerHTML=sortArrow(f);
  });
  tbody.innerHTML=filtered.map(e=>{
    const st=computeStatus(e);const sla=computeSLA(e);
    const slaH=sla===null?'<span class="sla sla-na">—</span>':sla>cfg.sla?`<span class="sla sla-warn">⚠ ${sla}d</span>`:`<span class="sla sla-ok">${sla}d</span>`;
    return `<tr class="${st==='Canceled'?'canceled':''}">
      ...
      <td><div class="row-act">
        <button class="bico" onclick="editEvent('${e.id}')" title="Edit"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="bico del" onclick="deleteEvent('${e.id}')" title="Delete"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>
      </div></td>
    </tr>`;
  }).join('');
}
...
async function generateEmail(){
  ...
  // ── FIX: only update the F-UP date, not registeredDate ──────────
  const toUpdate=[];
  events.forEach(e=>{
    const st=computeStatus(e);
    if(doFUP && st==='F-UP Needed'){
      e.fupDate=todayIso;   // update last F-UP date → resets SLA counter
      toUpdate.push(e);
    }
    if(doReg && st==='Registered'){
      e.requestedDate=todayIso;  // set Requested Date → moves to Requested
      // registeredDate is NEVER touched here
      toUpdate.push(e);
    }
  });
  ...
  const openingHtml=cfg.opening.replace('{date}',`<b>${todayFmt}</b>`).split('\n').filter(Boolean).map(l=>`<p style="margin:0 0 8px;font-family:Arial,sans-serif;font-size:13px;color:#1a1a1a">${l}</p>`).join('');
  const closingHtml=cfg.closing.split('\n').filter(Boolean).map(l=>`<p style="margin:0 0 5px;font-family:Arial,sans-serif;font-size:13px;color:#1a1a1a">${l}</p>`).join('');
  const sigHtml=cfg.signature.split('\n').filter(Boolean).map((l,i)=>i===0?`<p style="margin:0 0 2px;font-family:Arial,sans-serif;font-size:13px;font-weight:600;color:#1a1a1a">${l}</p>`:`<p style="margin:0;font-family:Arial,sans-serif;font-size:12px;color:#666;font-style:italic">${l}</p>`).join('');

  const rows=matched.map(e=>{
    const st=computeStatus(e);
    const sla=e.requestedDate?daysDiff(new Date(e.requestedDate),new Date(todayIso+'T00:00:00')):null;
    const rowBg={'F-UP Needed':'#fff3e0','Requested':'#e3f2fd','Registered':'#e8f5e9','Scheduled':'#f3e5f5','Concluded':'#eceff1','Canceled':'#ffebee'}[st]||'#fff';
    const stColor={'F-UP Needed':'#e65100','Requested':'#1565c0','Registered':'#2e7d32','Scheduled':'#6a1b9a','Concluded':'#37474f','Canceled':'#b71c1c'}[st]||'#333';
  ...
  const THSTYLE='padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em';
  const html='<html><body style="font-family:Arial,sans-serif;font-size:13px;color:#1a1a1a;max-width:720px;margin:0 auto;padding:20px">'
    ...
    +'<td style="'+THSTYLE+'">Event ID</td>'
    +'<td style="'+THSTYLE+'">Event Name</td>'
    +'<td style="'+THSTYLE+'">Speaker</td>'
    +'<td style="'+THSTYLE+'">Sector</td>'
    +'<td style="'+THSTYLE+'">Status</td>'
    +'<td style="'+THSTYLE+'">SLA Since Request</td>'
    +'<td style="'+THSTYLE+'">Observations</td>'
    +'</tr>'
    +rows
    +'</table>'
...
function openMailto(){
  ...
  if(!win){toast('Allow popups for this site.','err');return;}
  const body=window._emailHTML.replace(/<html>|<\/html>|<body[^>]*>|<\/body>/gi,'');
  win.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Email Preview</title>'
    +'<style>*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif}'
    +'.bar{background:#1a1a1a;color:#fff;padding:12px 18px;display:flex;align-items:center;gap:16px;position:sticky;top:0;z-index:10}'
    ...
    +'<script>function selAll(){var r=document.createRange();r.selectNode(document.getElementById("ec"));window.getSelection().removeAllRanges();window.getSelection().addRange(r);try{document.execCommand("copy");alert("Copied! Now paste into Outlook.");}catch(e){}}<\/script>'
    +'</body></html>');
  win.document.close();
}nJlZiI6ImFkemluY3d5bXNpZ2lnaWJzcHlsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM5NTkyODgsImV4cCI6MjA4OTUzNTI4OH0.Ii26AIWaTh1MvYABuqacmrFDYFTg0I_FqpdoTcdIRfo';
...
async function generateEmail(){
  ...
  const rows=matched.map(e=>{
    const st=computeStatus(e);
    const sla=e.requestedDate?daysDiff(new Date(e.requestedDate),new Date(todayIso+'T00:00:00')):null;
    const preS=e.ratingPreCall?'★'.repeat(e.ratingPreCall)+'☆'.repeat(5-e.ratingPreCall):'—';
    const callS=e.ratingCall?'★'.repeat(e.ratingCall)+'☆'.repeat(5-e.ratingCall):'—';
    const rowBg={'F-UP Needed':'#fff3e0','Requested':'#e3f2fd','Registered':'#e8f5e9','Scheduled':'#f3e5f5','Concluded':'#eceff1','Canceled':'#ffebee'}[st]||'#fff';
    const stColor={'F-UP Needed':'#e65100','Requested':'#1565c0','Registered':'#2e7d32','Scheduled':'#6a1b9a','Concluded':'#37474f','Canceled':'#b71c1c'}[st]||'#333';
    return `<tr style="background:${rowBg}">
      <td style="padding:8px 12px;border:1px solid #ddd;font-family:Arial,sans-serif;font-size:12px"><b>${e.id}</b></td>
      <td style="padding:8px 12px;border:1px solid #ddd;font-family:Arial,sans-serif;font-size:12px">${e.name}</td>
      <td style="padding:8px 12px;border:1px solid #ddd;font-family:Arial,sans-serif;font-size:12px">${e.sector||'—'}</td>
      <td style="padding:8px 12px;border:1px solid #ddd;font-family:Arial,sans-serif;font-size:12px;font-weight:700;color:${stColor}">${st}</td>
      <td style="padding:8px 12px;border:1px solid #ddd;font-family:Arial,sans-serif;font-size:12px">${sla!==null?sla+' days':'—'}</td>
      <td style="padding:8px 12px;border:1px solid #ddd;font-family:Arial,sans-serif;font-size:12px;color:#F15A22">${preS}</td>
      <td style="padding:8px 12px;border:1px solid #ddd;font-family:Arial,sans-serif;font-size:12px;color:#F15A22">${callS}</td>
    </tr>`;
  }).join('');

  const html=`<html><body style="font-family:Arial,sans-serif;font-size:13px;color:#1a1a1a;max-width:700px;margin:0 auto;padding:20px">
<div style="background:#1a1a1a;padding:16px 20px;border-radius:6px 6px 0 0;margin-bottom:20px">
  <span style="color:#F15A22;font-weight:700;font-size:16px">itaú</span><span style="color:#fff;font-weight:700;font-size:16px"> BBA</span>
  <span style="color:#888;font-size:12px;margin-left:12px">Equity Research — Event Tracker</span>
</div>
${openingHtml}
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:12px;width:100%;border-color:#ddd;margin:14px 0">
<tr style="background:#1a1a1a;color:#fff">
  <td style="padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em">Event ID</td>
  <td style="padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em">Event Name</td>
  <td style="padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em">Sector</td>
  <td style="padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em">Status</td>
  <td style="padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em">SLA</td>
  <td style="padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em">Pre-Call</td>
  <td style="padding:9px 12px;border:1px solid #333;font-family:Arial,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em">Call</td>
</tr>
${rows}
</table>
<div style="border-top:2px solid #F15A22;margin-top:20px;padding-top:14px">
...
<br>${sigHtml}
  ...
  document.getElementById('emailPreviewWrap').innerHTML=`<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><span style="font-size:12px;color:var(--text-s)">To: <b>${to||'(no recipient)'}</b> · ${matched.length} event(s)</span></div><iframe id="prevFrame" style="width:100%;height:460px;border:1px solid var(--border);border-radius:4px"></iframe>`;
...
async function saveSettings(){
  ...
  cfg.to=document.getElementById('cfg-to').value.trim();cfg.cc=document.getElementById('cfg-cc').value.trim();
...
async function changePassword(){
  ...
  if(!cur||!neu){toast('Fill current and new password.','err');return;}if(neu!==conf){toast('Passwords do not match.','err');return;}if(neu.length<6){toast('Min 6 characters.','err');return;}
  const users=await getAllUsers();const idx=users.findIndex(u=>u.username===currentUser.username);
  if(await sha256(cur)!==users[idx].passwordHash){toast('Current password is incorrect.','err');return;}
  users[idx].passwordHash=await sha256(neu);await saveUserToDb(users[idx]);
  ['pw-current','pw-new','pw-confirm'].forEach(id=>document.getElementById(id).value='');toast('Password changed.','ok');
}
...
async function saveSupabase(){
  ...
  if(initSupabase()){await dbLoadAll();subscribeRealtime();updateSyncStatus();renderConnect();toast('Connected!','ok');}
  else{toast('Connection failed.','err');}
}
function disconnectSupabase(){localStorage.removeItem('crm_supabase');_supabase=null;if(realtimeSub){realtimeSub.unsubscribe();realtimeSub=null;}updateSyncStatus();renderConnect();toast('Disconnected.','ok');}

// ── Users ────────────────────────────────────────────────────────
async function renderUsers(){
  const users=await getAllUsers();
  document.getElementById('userTableBody').innerHTML=users.map(u=>`<tr><td>${u.username}</td><td>${u.displayName}</td><td>${u.initials}</td><td><span class="pill ${u.role==='admin'?'p-Requested':'p-Registered'}">${u.role}</span></td><td><div class="row-act">${u.username!==currentUser.username?`<button class="bico del" onclick="deleteUser('${u.username}')"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></button>`:'<span style="font-size:10px;color:var(--text-xs)">you</span>'}</div></td></tr>`).join('');
}
async function addUser(){
  ...
  const users=await getAllUsers();if(users.find(u=>u.username.toLowerCase()===uname.toLowerCase())){toast('Username exists.','err');return;}
  ...
  ['new-username','new-displayname','new-initials','new-password'].forEach(id=>document.getElementById(id).value='');document.getElementById('new-role').value='user';
...
function importData(){const input=document.createElement('input');input.type='file';input.accept='.json';input.onchange=e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=async ev=>{try{const d=JSON.parse(ev.target.result);if(d.events)events=d.events;if(d.speakers)speakers=d.speakers;if(d.cfg)cfg={...CFG_DEFAULTS,...d.cfg};if(isOnline()){for(const ev of events)await dbSaveEvent(ev);}saveLocal();renderDashboard();toast(`Imported ${events.length} events.`,'ok');}catch{toast('Invalid JSON.','err');}};r.readAsText(file);};input.click();}
