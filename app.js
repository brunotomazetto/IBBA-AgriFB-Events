/* ═══════════════════════════════════════════════════════════════
   Event Tracker CRM — app.js v3
   Itaú BBA theme · login · ratings · localStorage
═══════════════════════════════════════════════════════════════ */
...
async function seedAdminIfNeeded() {
  if (!getUsers().length) {
    ...
    saveUsers([{ username:'admin', displayName:'Bruno Tomazetto', initials:'BT', role:'admin', passwordHash:hash }]);
  }
}
async function tryLogin(username, password) {
  const user = getUsers().find(u => u.username.toLowerCase() === username.toLowerCase());
  if (!user) return false;
  if (await sha256(password) !== user.passwordHash) return false;
  currentUser = user;
...
let activeFilter = 'All', editEvtId = null, editSpkId = null;
...
function starHTML(value, fieldName, evtId) {
  ...
  for (let i = 1; i <= 5; i++) {
    h += `<span class="star${i<=(value||0)?' on':''}" onclick="setRating('${evtId}','${fieldName}',${i})" onmouseover="hoverStars(this,${i})" onmouseout="resetStars(this)">★</span>`;
  }
  return h + '</div>';
}
function setRating(id, field, val) {
  const ev = events.find(e => e.id===id); if (!ev) return;
  ev[field] = ev[field]===val ? 0 : val;
  save(); renderEvents(); renderDashboard();
}
function hoverStars(el, n) {
  el.closest('.stars').querySelectorAll('.star').forEach((s,i) => s.classList.toggle('hover', i<n));
}
function resetStars(el) {
  el.closest('.stars').querySelectorAll('.star').forEach(s => s.classList.remove('hover'));
}
...
function updateModalStars(fieldId) {
  ...
  document.getElementById(fieldId+'-stars')?.querySelectorAll('.star-btn').forEach((s,i) => {
    s.classList.toggle('on', i<val); s.textContent = i<val ? '★' : '☆';
  });
}
...
function showApp() {
  ...
  const usersTab = document.getElementById('snav-users');
  if (usersTab) usersTab.style.display = currentUser.role==='admin' ? '' : 'none';
  renderDashboard();
  setPage('dashboard');
}
...
function setPage(p) {
  ...
  const snav = document.getElementById('snav-'+p);
  if (!page) return;
...
function renderDashboard() {
  ...
  document.getElementById('userAvatarBtn').textContent = currentUser?.initials||cfg.initials||'BT';
...
function saveEvent() {
  ...
  if(editEvtId){const idx=events.findIndex(e=>e.id===editEvtId);events[idx]={...events[idx],...data};toast('Event updated.','ok');}
  else{events.push({id:nextEvtId(),registeredDate:isoToday(),...data});toast('Event created.','ok');}
  save();closeModal('evtModal');renderEvents();renderDashboard();
}
function deleteEvent(id){if(!confirm('Delete this event?'))return;events=events.filter(e=>e.id!==id);save();renderEvents();renderDashboard();toast('Event deleted.','ok');}
...
function saveSpeaker(){const name=document.getElementById('sf-name').value.trim();if(!name){toast('Name is required.','err');return;}const data={name,company:document.getElementById('sf-company').value.trim(),sector:document.getElementById('sf-sector').value,email:document.getElementById('sf-email').value.trim()};if(editSpkId){const idx=speakers.findIndex(s=>s.id===editSpkId);speakers[idx]={...speakers[idx],...data};toast('Speaker updated.','ok');}else{speakers.push({id:nextSpkId(),...data});toast('Speaker created.','ok');}save();closeModal('spkModal');renderSpeakers();}
function deleteSpeaker(id){if(!confirm('Delete this speaker?'))return;speakers=speakers.filter(s=>s.id!==id);save();renderSpeakers();toast('Speaker deleted.','ok');}
...
function generateEmail(){
  ...
  events.forEach(e=>{const st=computeStatus(e);if(doFUP&&st==='F-UP Needed')e.fupDate=todayIso;if(doReg&&st==='Registered')e.requestedDate=todayIso;});
  if(doFUP||doReg)save();
...
function saveSettings(){
  ...
  save();document.getElementById('userAvatarBtn').textContent=currentUser.initials;toast('Settings saved.','ok');
}
...
async function changePassword(){
  ...
  users[idx].passwordHash=await sha256(neu);saveUsers(users);['pw-current','pw-new','pw-confirm'].forEach(id=>document.getElementById(id).value='');toast('Password changed.','ok');
}
function deleteUser(username){if(!confirm(`Delete user "${username}"?`))return;saveUsers(getUsers().filter(u=>u.username!==username));renderUsers();toast(`User "${username}" deleted.`,'ok');}
...
function importData(){const input=document.createElement('input');input.type='file';input.accept='.json';input.onchange=e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>{try{const d=JSON.parse(ev.target.result);if(d.events)events=d.events;if(d.speakers)speakers=d.speakers;if(d.cfg)cfg={...CFG_DEFAULTS,...d.cfg};save();renderDashboard();toast(`Imported ${events.length} events.`,'ok');}catch{toast('Invalid JSON.','err');}};r.readAsText(file);};input.click();}
